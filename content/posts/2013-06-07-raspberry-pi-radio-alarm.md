...
My RasPi is running [Raspian][] right now, but any Linux should do. You
...
package lists before installing.
...
Now, ensure that cron is running (`ps aux | grep cron` should give you
a hint) and type `crontab -e` to enter your cronjob (preferably as a
...
background. The easiest way of doing so is via `killall mplayer`, which is not
the finest way of doing things but should do the job.
